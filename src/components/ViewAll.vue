<template>
    <div class="px-4 mb-12">
        <input type="text"
            ...
            placeholder="Search" v-model="projInput">
        <div class="px-4 my-4 max-w-[650px] mx-auto md:px-1">
            <p class="archivo font-semibold text-lg text-center mb-4 mt-8">There are no rules with these projects. Use any
                languages, libraries, or frameworks you want to recreate and learn from them.</p>
        </div>
    </div>
...
export default {
    ...
    computed: {
        filteredProjects() {
            return this.projects.filter((project) => project.title.toLowerCase().includes(this.projInput.toLowerCase()))
        }
    }
}
</script>
